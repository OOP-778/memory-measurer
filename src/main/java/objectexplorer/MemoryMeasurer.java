...
/**
 * A utility that can be used to measure the memory footprint of an arbitrary object graph. In a nutshell, the user gives a root object, and this class
...
public class MemoryMeasurer {
    ...
    /**
     * Measures the memory footprint, in bytes, of an object graph. The object graph is defined by a root object and whatever object can be reached through
     * that, excluding static fields, {@code Class} objects, and fields defined in {@code enum}s (all these are considered shared values, which should not
     ...
     * Measures the memory footprint, in bytes, of an object graph. The object graph is defined by a root object and whatever object can be reached through
     * that, excluding static fields, {@code Class} objects, and fields defined in {@code enum}s (all these are considered shared values, which should not
     * contribute to the cost of any single object graph), and any object for which the user-provided predicate returns {@code false}.
     *
     * @param rootObject     the root object that defines the object graph to be measured
     * @param objectAcceptor a predicate that returns {@code true} for objects to be explored (and treated as part of the object graph), or {@code false} to
    ...
    private static class MemoryMeasurerVisitor implements ObjectVisitor<Long> {
        ...
        public Traversal visit(Chain chain) {
            if (this.predicate.apply(chain)) {
                ...
                this.memory += instrumentation.getObjectSize(o);
                if (Enum.class.isAssignableFrom(o.getClass())) {
                    this.memory -= costOfBareEnumConstant;
                }
