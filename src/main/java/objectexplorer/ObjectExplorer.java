...
/**
 * A depth-first object graph explorer. The traversal starts at a root (an {@code Object}) and explores any other reachable object (recursively) or primitive
 * value, excluding static fields from the traversal. The traversal is controlled by a user-supplied {@link ObjectVisitor}, which decides for each explored path
 * whether to continue exploration of that path, and it can also return a value at the end of the traversal.
 */
public class ObjectExplorer {
    static final Predicate<Chain> notEnumFieldsOrClasses = chain ->
        !(Enum.class.isAssignableFrom(chain.getValueType())
    ...
    /**
     * Explores an object graph (defined by a root object and whatever is reachable through it, following non-static fields) while using an
     ...
     * @param visitor    a visitor that is notified for each explored path and decides whether to continue exploration of that path, and constructs a return
     *                   value at the end of the exploration
    ...
    public static <T> T exploreObject(Object rootObject,
        ...
        final Deque<Chain> stack = new ArrayDeque<Chain>(32);
        if (rootObject != null) {
            stack.push(Chain.root(rootObject));
        }

        while (!stack.isEmpty()) {
            final Chain chain = stack.pop();
            //the only place where the return value of visit() is considered
            final ObjectVisitor.Traversal traversal = visitor.visit(chain);
            switch (traversal) {
                case SKIP:
                    continue;
                case EXPLORE:
            ...
            @Nonnull
            final Object value = chain.getValue();
            final Class<?> valueClass = value.getClass();
            if (valueClass.isArray()) {
                final boolean isPrimitive = valueClass.getComponentType().isPrimitive();
                for (int i = Array.getLength(value) - 1; i >= 0; i--) {
                    final Object childValue = Array.get(value, i);
                    if (isPrimitive) {
                ...
                for (final Field field : getAllFields(value)) {
                    ...
                    final Chain extendedChain = chain.appendField(field, childValue);
                    if (isPrimitive) {
                        if (features.contains(Feature.VISIT_PRIMITIVES)) {
    ...
    private static Iterable<Field> getAllFields(Object o) {
        final List<Field> fields = Lists.newArrayListWithCapacity(8);
    ...
    public static class AtMostOncePredicate implements Predicate<Chain> {
        ...
        public boolean apply(Chain chain) {
            final Object o = chain.getValue();
            return o instanceof Class<?> || this.interner.add(o);
        }
    }
}
